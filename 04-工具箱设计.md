...
分布式场景下 T2 还要扫 RPC 的入边和出边（如 Dubbo 的 `@DubboService` / `@DubboReference`），并衍生出 **T2.6 跨服务调用图**：全仓库静态 join + 注册中心对账，详见 [06-分布式与Dubbo.md](06-分布式与Dubbo.md)。
...
分布式场景下收口点增加 RPC 框架的拦截器（如 Dubbo provider/consumer Filter）；如果挂了 OTel 自动埋点 agent，span 统计可以直接替代收口点插桩（见 [06-分布式与Dubbo.md](06-分布式与Dubbo.md)）。
...
调用图还有第三个数据源：**CI 测试时采集**（跑测试套件时挂 agent/tracepoint 采真实调用边），介于静态分析和生产 trace 之间——零生产风险、CI 自动更新，静态图连不上的反射/DI 断点可以由它补真实的边，见 [08-梳理阶段实战.md](08-梳理阶段实战.md) 第三节。
...
输出端遵循"一份数据、两个投影"：数据层（CSV/YAML 事实源）→ 模型投影（stable ID + 显式边的卡片）→ 人类投影（mermaid/treemap/HTML 渲染），生成顺序不可逆转，详见 [07-面向人与模型的表示.md](07-面向人与模型的表示.md)。
...
例外：它的**棘轮版本**（存量违规先豁免为 baseline，新增违规直接拦）应该在梳理期就提前上线——违规数只许降不许升，见 [08-梳理阶段实战.md](08-梳理阶段实战.md) 第四节。
...
Java/Spring 栈的具体落地细节（现成捷径和栈特有的坑）见 [05-Java-Spring落地指南.md](05-Java-Spring落地指南.md)；分布式/Dubbo 场景对各工具的扩展和 trace 方案见 [06-分布式与Dubbo.md](06-分布式与Dubbo.md)。
